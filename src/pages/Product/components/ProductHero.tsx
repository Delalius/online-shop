...
export const ProductHero = ({
  ...
  return (
    <section className="overflow-hidden rounded-[2rem] border border-white/70 bg-white/90 shadow-[0_20px_80px_-45px_rgba(15,23,42,0.45)] backdrop-blur">
      <div className="grid gap-8 p-5 sm:p-8 xl:grid-cols-[minmax(0,1.15fr)_minmax(520px,0.85fr)] xl:items-start">
        <div className="rounded-[1.75rem] bg-[linear-gradient(145deg,#f8fbff_0%,#e2efff_100%)] p-6 sm:p-8">
          <div className="mb-5 flex items-center justify-between gap-3">
            <span className="rounded-full bg-white/85 px-4 py-1.5 text-sm font-semibold capitalize text-sky-700 shadow-sm">
              {product.category}
            </span>
        ...
        <div className="flex flex-col justify-between gap-6">
          <div>
            <div className="mb-4 flex flex-wrap items-center gap-3">
              ...
              <span className="text-sm text-slate-500">{reviewCount} отзывов</span>
            ...
            <p className="mt-4 max-w-3xl text-base leading-7 text-slate-600 xl:text-lg">
              {product.description}
            </p>
          </div>

          <div className="rounded-[1.75rem] border border-slate-200 bg-slate-50/80 p-5 xl:p-6">
            <div className="flex flex-wrap items-end justify-between gap-4">
              <div>
                <p className="text-sm text-slate-500">Цена сегодня</p>
                <p className="mt-1 text-4xl font-bold tracking-tight text-slate-950 xl:text-5xl">
                  ${product.price.toFixed(2)}
                </p>
              </div>
              <div className="rounded-2xl bg-white px-4 py-3 text-sm text-slate-500 shadow-sm">
                Доставка по Украине и ЕС
              </div>
            </div>
            ...
            <div className="mt-5 grid gap-3 sm:grid-cols-3">
              ...
              <div className="rounded-2xl bg-white p-4 shadow-sm">
                <p className="text-sm text-slate-500">Код товара</p>
                <p className="mt-1 font-semibold text-slate-900">{productCode}</p>
              </div>
              <div className="rounded-2xl bg-white p-4 shadow-sm">
                <p className="text-sm text-slate-500">Уже в корзине</p>
                <p className="mt-1 font-semibold text-slate-900">
                  {currentQuantity > 0 ? `${currentQuantity} шт.` : "Пока нет"}
                </p>
              </div>
            </div>
            ...
            <div className="mt-5 flex flex-col gap-3 sm:flex-row">
              ...
              <div className="flex items-center justify-between rounded-2xl border border-slate-200 bg-white px-3 py-2 sm:min-w-[180px]">
                <button
                  onClick={onDecrease}
