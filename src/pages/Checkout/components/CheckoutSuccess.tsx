...
export const CheckoutSuccess = ({ form }: Props) => {
  return (
    <div className="min-h-screen bg-[radial-gradient(circle_at_top,_rgba(14,165,233,0.15),_transparent_28%),linear-gradient(180deg,#f8fafc_0%,#eef6ff_55%,#f8fafc_100%)] px-4 py-8 sm:px-6 sm:py-12">
      <div className="mx-auto max-w-3xl rounded-[2rem] border border-white/70 bg-white/90 p-8 text-center shadow-[0_20px_80px_-45px_rgba(15,23,42,0.45)] backdrop-blur sm:p-12">
        ...
        <p className="mt-4 text-base leading-7 text-slate-600">
          Мы получили ваши данные и скоро свяжемся для подтверждения. Письмо с
          деталями отправим на {form.email || "указанный email"}.
        </p>
        <div className="mt-8 grid gap-4 rounded-[1.5rem] bg-slate-50 p-5 text-left sm:grid-cols-2">
          <div>
            <p className="text-sm text-slate-500">Получатель</p>
          ...
          <div>
            <p className="text-sm text-slate-500">Доставка</p>
            <p className="mt-1 font-semibold text-slate-900">
              {form.deliveryMethod === "courier" ? "Курьером" : "Самовывоз"}
            </p>
          </div>
          <div>
